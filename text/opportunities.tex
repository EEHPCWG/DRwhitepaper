...
%How about advance-reservation capability in schedulers? I (Mehdi) suggest to mention this capability as well, if you agree.
%The same for grid computing.

%How about multi-dimensional resource management and scheduling (multi-resource)? I have seen there are very good discussions about scheduling. We may take into account multi-resource scheduling. This means this scheduling takes into account not only CPU, but also other system resources like Memory, GPU, network input/network output, storage, etc. Even software/application can be a resource.
